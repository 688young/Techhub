...
async function start() {
  ...
  app.get('/services', isAuth, wrap(async (req, res) => {
  ...
  app.get('/order/:serviceId', isAuth, wrap(async (req, res) => {
  ...
  app.post('/order', isAuth, wrap(async (req, res) => {
    ...
    await run('INSERT INTO orders (user_id, customer_name, customer_email, service, service_option, price, phone, payment_network, confirmation_code) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)',
      [user.id, user.username, user.email, service.name, optionName, finalPrice, phone, payment_network, code]);

    const order = await get('SELECT id FROM orders WHERE user_id = $1 AND confirmation_code = $2 ORDER BY id DESC LIMIT 1', [user.id, code]);
  ...
  app.post('/order/confirm', isAuth, wrap(async (req, res) => {
  ...
  app.get('/dashboard', isAuth, wrap(async (req, res) => {
  ...
  app.get('/admin', isAuth, isAdmin, wrap(async (req, res) => {
  ...
  app.post('/admin/service/add', isAuth, isAdmin, wrap(async (req, res) => {
    const { name, description, long_description, price, category, has_options } = req.body;
    await run('INSERT INTO services (name, description, long_description, price, category, has_options) VALUES ($1, $2, $3, $4, $5, $6)',
      [name, description, long_description || '', parseFloat(price), category, has_options === 'on' ? 1 : 0]);
    res.redirect('/admin');
  }));

  app.post('/admin/service/edit', isAuth, isAdmin, wrap(async (req, res) => {
    const { id, name, description, long_description, price, category, has_options } = req.body;
    await run('UPDATE services SET name=$1, description=$2, long_description=$3, price=$4, category=$5, has_options=$6 WHERE id=$7',
      [name, description, long_description || '', parseFloat(price), category, has_options === 'on' ? 1 : 0, id]);
    res.redirect('/admin');
  }));

  app.post('/admin/service/toggle', isAuth, isAdmin, wrap(async (req, res) => {
    const { id } = req.body;
  ...
  app.post('/admin/service/delete', isAuth, isAdmin, wrap(async (req, res) => {
    const { id } = req.body;
  ...
  app.post('/admin/option/add', isAuth, isAdmin, wrap(async (req, res) => {
  ...
  app.post('/admin/option/edit', isAuth, isAdmin, wrap(async (req, res) => {
  ...
  app.post('/admin/option/toggle', isAuth, isAdmin, wrap(async (req, res) => {
    const { id } = req.body;
  ...
  app.post('/admin/option/delete', isAuth, isAdmin, wrap(async (req, res) => {
    const { id } = req.body;
    await run('DELETE FROM service_options WHERE id = $1', [id]);
    res.redirect('/admin');
  }));

  app.post('/admin/order/status', isAuth, isAdmin, wrap(async (req, res) => {
    const { id, status } = req.body;
    await run('UPDATE orders SET status = $1 WHERE id = $2', [status, id]);
    res.redirect('/admin');
  }));

  app.post('/admin/message/read', isAuth, isAdmin, wrap(async (req, res) => {
    const { id } = req.body;
    await run('UPDATE messages SET is_read = 1 WHERE id = $1', [id]);
    res.redirect('/admin');
  }));

  app.post('/admin/message/delete', isAuth, isAdmin, wrap(async (req, res) => {
    const { id } = req.body;
    await run('DELETE FROM messages WHERE id = $1', [id]);
    res.redirect('/admin');
  }));

  app.post('/admin/user/delete', isAuth, isAdmin, wrap(async (req, res) => {
    const { id } = req.body;
  ...
  app.post('/admin/payment/verify', isAuth, isAdmin, wrap(async (req, res) => {
    const { id } = req.body;
    await run('UPDATE orders SET payment_verified = 1 WHERE id = $1', [id]);
    res.redirect('/admin');
  }));

  app.post('/admin/payment/unverify', isAuth, isAdmin, wrap(async (req, res) => {
    const { id } = req.body;
